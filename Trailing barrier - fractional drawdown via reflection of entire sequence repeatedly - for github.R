...
X_double_reflected <- array(dim=c(nSim,N,R))
...
X_double_reflected[,j,1] <- X[,j] * pmin(1,B_running_min_old_double[,j,1])   
Y_running_min_old_double[,j,1] <- ifelse(X_double_reflected[,j,1] < Y_running_min_old_double[,j-1,1], 
                                         X_double_reflected[,j,1], Y_running_min_old_double[,j-1,1])

}

for (j in 2:N){ # Then reflect result at the bottom:

  B_old_double_bottom[,j,1] <- b_as_matrix[,j]/X_double_reflected[,j,1] # from above
  ...
  X_double_reflected[,j,1] <- X_double_reflected[,j,1] * pmax(1,B_running_max_old_double[,j,1])
  Y_running_max_old_double[,j,1] <- ifelse(X_double_reflected[,j,1] > Y_running_max_old_double[,j-1,1],
                                           X_double_reflected[,j,1], Y_running_max_old_double[,j-1,1])
...
for (s in 2:R){
  ...
  for (j in 2:N){  # top reflection
    ...
    B_old_double_top[,j,s] <- 1 /X_double_reflected[,j,s-1]  # has to be s-1 here, not found for s yet
    ...
    X_double_reflected[,j,s] <- X_double_reflected[,j,s-1] * pmin(1,B_running_min_old_double[,j,s])
    Y_running_min_old_double[,j,s] <- ifelse(X_double_reflected[,j,s] < Y_running_min_old_double[,j-1,s],
                                             X_double_reflected[,j,s], Y_running_min_old_double[,j-1,s])

  }

  for (j in 2:N){ # bottom reflection

    B_old_double_bottom[,j,s] <- b_as_matrix[,j]/X_double_reflected[,j,s] # s as final subscript here, using output from directly above
    ...
    X_double_reflected[,j,s] <- X_double_reflected[,j,s] * pmax(1,B_running_max_old_double[,j,s])
    Y_running_max_old_double[,j,s] <- ifelse(X_double_reflected[,j,s] > Y_running_max_old_double[,j-1,s],
                                             X_double_reflected[,j,s], Y_running_max_old_double[,j-1,s])
