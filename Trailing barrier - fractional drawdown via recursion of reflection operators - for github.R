...
Y_old_double <- array(dim=c(nSim,N,R))
...
Y_old_double[,j,1] <- X[,j] * pmin(1,B_running_min_old_double[,j,1])   
Y_running_min_old_double[,j,1] <- ifelse(Y_old_double[,j,1] < Y_running_min_old_double[,j-1,1], 
                                         Y_old_double[,j,1], Y_running_min_old_double[,j-1,1])

}

for (j in 2:N){ # Then reflect result at the bottom:

  B_old_double_bottom[,j,1] <- b_as_matrix[,j]/Y_old_double[,j,1] # from above
  ...
  Y_old_double[,j,1] <- Y_old_double[,j,1] * pmax(1,B_running_max_old_double[,j,1])
  Y_running_max_old_double[,j,1] <- ifelse(Y_old_double[,j,1] > Y_running_max_old_double[,j-1,1],
                                           Y_old_double[,j,1], Y_running_max_old_double[,j-1,1])
...
# Sorry about this clunky code. Couldn't pass a matrix to a function, and "repeat" with a break condition 
# of (max(Y_old_double[33,])- min(Y_old_double[33,] <= 1-b)){break doesn't seem to work.
...
for (s in 2:R){
  ...
  for (j in 2:N){  # top reflection
    ...
    B_old_double_top[,j,s] <- 1 /Y_old_double[,j,s-1]  # has to be s-1 here, not found for s yet
    ...
    Y_old_double[,j,s] <- Y_old_double[,j,s-1] * pmin(1,B_running_min_old_double[,j,s])
    Y_running_min_old_double[,j,s] <- ifelse(Y_old_double[,j,s] < Y_running_min_old_double[,j-1,s],
                                             Y_old_double[,j,s], Y_running_min_old_double[,j-1,s])

  }

  for (j in 2:N){ # bottom reflection

    B_old_double_bottom[,j,s] <- b_as_matrix[,j]/Y_old_double[,j,s] # s as final subscript here, using output from directly above
    ...
    Y_old_double[,j,s] <- Y_old_double[,j,s] * pmax(1,B_running_max_old_double[,j,s])
    Y_running_max_old_double[,j,s] <- ifelse(Y_old_double[,j,s] > Y_running_max_old_double[,j-1,s],
                                             Y_old_double[,j,s], Y_running_max_old_double[,j-1,s])
