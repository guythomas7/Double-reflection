...
# Setting parameters 
D <- 2 # number of times drawdown traverse from 1 to b (top to bottom)
R <- D + 1 # We go from (2 to R) below, so this gives D recursions 
...
# Now construct the drawdown of the Y_old, "GBM reflected at trailing barrier", by recursive reflection 
# of each data point of the original GBM at 1 and b.


for (j in 2:N){ 
  
  X_double_reflected[,j,1] <- X[,j] # set the data point for reflection to the original GBM, to start with
  
  for (s in 2:R){ 
    
    # First reflection at top
